  ...
  return (
    ...
    <table className="passedOrder">
      ...
      <tbody>
          {data2.map((item)=>(
              <tr key={item.id}>
                  <td className='flex justify-center'><img className='h-[40px] w-[40px] rounded-full' src={item.imageUrl} alt="" /></td>
                  <td>{item.name}</td>
                  <td>{item.date}</td>
                  <td>{item.montant}</td>
                  <td><button className="bg-secondary text-white px-3 py-2 rounded-lg">Contact</button></td>
              </tr>
          ))}
